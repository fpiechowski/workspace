...
var commandHelpSpecs = map[string]commandHelp{
	...
	"workspace message": h(
		"Send durable notes, questions, answers or status messages to agent identities.",
		"workspace message send --to agent_parent --kind question --body-file question.md",
	),
	"workspace message send": h(
		"Send the contents of a file as a durable message. Address messages to Agent IDs, not native thread IDs.",
		"workspace message send --to agent_parent --kind note --body-file note.md",
	),
	"workspace inbox": h(
	...
	"workspace inbox list": h(
		"List pending messages. Use --all to include messages already acknowledged.",
		"workspace inbox list --agent agent_parent --all",
	),
	"workspace inbox read": h(
		"Read one message without acknowledging it.",
		"workspace inbox read msg_01",
		requiredArgument("message", "Message ID to read."),
	),
	"workspace inbox ack": h(
		"Read and record receipt of one message. Acknowledgement does not accept the associated task or handoff.",
		"workspace inbox ack msg_01",
		requiredArgument("message", "Message ID to acknowledge."),
	),
	"workspace inbox wait": h(
		"Wait for messages without holding the project lock. The command returns when a message arrives or the timeout expires.",
		"workspace inbox wait --timeout 60",
	),
	"workspace handoff": h(
	...
	"workspace handoff submit": h(
		"Copy explicitly named artifacts and enqueue a handoff result atomically. A summary file is required; check receipts can be attached for verification evidence.",
		"workspace handoff submit --task task_01 --summary-file work-products/SUMMARY.md --artifact work-products/IMPLEMENTATION.md",
	),
...
var flagHelpSpecs = map[string]map[string]string{
	...
	"workspace message send": {
		...
		"to":        "Recipient Agent ID or persona name (required).",
		"kind":      "Message kind: note, question, answer or status.",
		"reply-to":  "Original message ID when this is a reply.",
	},
	"workspace inbox list": {"agent": "Inbox owner; defaults to the current agent or orchestrator.", "all": "Include acknowledged messages."},
	"workspace inbox wait": {"timeout": "Maximum wait in seconds.", "agent": "Inbox owner."},
	"workspace handoff submit": {
		"to":           "Parent or orchestrator Agent ID.",
